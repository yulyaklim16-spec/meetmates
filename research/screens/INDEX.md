...
**Зачем этот файл.** Скрин — доказательство конкретного наблюдения, а не иллюстрация.
Здесь у каждого есть подпись: что на нём видно и что из этого следует.

---

## Какие решения держатся на этих экранах

| Решение | На чём держится |
|---|---|
| «Не дейтинг» — гарантия, а не обещание `D-05` | `tinder--onboarding--10-looking-for` — «Найти друзей» одной из шести карточек |
| Гейт верификации стоит наоборот, чем у Tinder `D-03` | `tinder--chat--29-message-settings-verified` — фильтр входящих |
| Репутация из фактов — свободная ниша | `tinder--profile--30-my-profile-gold` — ни одного сигнала в профиле |
| Report / Block с любого экрана | `meetup--profile--27-privacy` — одна строка, блок-листа нет |
| Ни намёка на деньги в v1 `D-28` | `meetup--paywall--12-onboarding-paywall` — восьмой шаг из девяти |
| Рейтинга человека не будет никогда | `meetup--event--19-detail-top` — балл стоит на группе |
| Создание запроса в один шаг `D-22` | `meetup--events--22-create-group-step1` — шесть шагов до первого события |
| Приватная обратная связь | `meetup--safety--28-report-event` — «we won't share this with the organizer» |
| Бейджа присутствия не делаем `D-06` | `bumble-bff--app--04-flow-strip` — «Last active 21 minutes ago» |
| Плотность — главный риск `D-02` | `meetup--app--29-flow-strip` — 326 участников против 2–6 в Киеве |

---

## Условия съёмки

Веб — мобильный вьюпорт **390 × 844** (наш базовый макет), Playwright.
Съёмка с телефона — нативное разрешение устройства.
Разбор — [`../competitor-analysis.md`](../competitor-analysis.md).
