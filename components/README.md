...
| Решение | Компонент, который обязан это держать |
|---|---|
| Сигналы доверия — до первого сообщения | `SwipeCard` показывает все четыре, а не только Verified |
| Счётчик двухфазный `D-16` | `SpotsCounter`: `going` → `confirmed`, и `spots left` не растёт |
| Ограничение только на свой пол `D-25` | `WhoCanJoinPicker` отключает чужой пол, а не прячет |
| Расстояние округлено | `DistanceLabel` не принимает точных координат в принципе |
| Пустое состояние — не край | `EmptyState` обязан иметь действие, а не только текст |

**Готовность этапа:** у каждого компонента описаны все состояния — default · hover · active ·
focus · disabled · loading · error · empty. Отсутствующее состояние = баг на сборке.
