...
flowchart TD
    ...
    WHOMADE --> Q17{"Хочу повторить с ними же?"}
...
flowchart TD
    ...
    I -->|да| L{"Запрос был помечен как повторяющийся?"}

    L -->|нет| M["Создание запроса"]
    M --> N["Карточка запроса: свой с откликами"]
    N --> O{"Пришёл кто-то из прошлого состава?"}
    L -->|да| P["Карточка запроса: серия с историей"]
...
3. **Запрос был помечен как повторяющийся?** — серию нельзя создать отдельно, только из запроса (`D-15`).
   Ветка «нет» — самая дорогая: человек проходит создание заново, то есть job не закрыт.
4. **Пришёл кто-то из прошлого состава?** — вторичный срез метрики повтора (§11).

**Состояния.** Окно check-in закрыто · свой запрос с откликами · серия с историей · чат серии.
...
| **R4 ломается на развилке «запрос не был помечен повторяющимся»** | Проверить, не должен ли повтор предлагаться после первой встречи автоматически `[?]` |
