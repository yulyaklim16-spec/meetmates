...
flowchart TD
    ...
    NotCounted --> Steps{Есть шаг, который могу сделать сегодня?}
    Counted -->|да| Grew["Мой профиль: счётчик встреч вырос"]
    Grew --> Card["Профиль другого человека: счётчик виден на карточке"]
    ...
    Verify --> Card
    Edit --> Grew
    Feed --> Later(["Job не закрыт: счётчик растёт только встречами"])
...
2. **Счётчик вырос?** — присутствие засчитывается, если отметил хотя бы один другой
   участник (§5.5.1). Ступеней профиля в продукте нет `D-49`: доверие растёт
   верификацией и подтверждёнными встречами, а не уровнем.
3. **Есть шаг на сегодня?** — верификация и фотографии зависят только от человека.

**Состояния.** Окно закрыто · счётчик не вырос · вырос.

**Тупиков нет.** Единственный терминал без успеха — «счётчик растёт только встречами» —
