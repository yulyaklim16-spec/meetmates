# User flows — MeetMates

> Экраны и состояния — из [`sitemap.md`](./sitemap.md), новых не придумано.
> Jobs — из [`research/jtbd.md`](./research/jtbd.md), решения — [`DECISIONS.md`](./DECISIONS.md).
>
> **Как читать.** Прямоугольник — экран или его состояние, ромб — точка решения,
> закруглённый — начало или конец пути. Тупики помечены словом **ТУПИК** в тексте узла:
> это места, где человек может застрять, и именно они важнее happy-path.

---

## M1 — компания для планов и продолжение

> Когда мне есть куда пойти, но не с кем, я хочу, чтобы у моих планов была компания,
> а у компании — продолжение.

**Кто:** Даша, primary. **Основной путь — через чат** `D-37`: колода → взаимный свайп →
диалог → договорились. Ветка через ленту запросов — второй путь, детерминированный.
Путь считается от **пустой колоды**: на старте она увидит именно её (`D-22`).

```mermaid
flowchart TD
    START(["Открыла приложение после онбординга"]) --> DECK["Колода"]
    DECK --> DLOAD["Колода: загрузка"]
    DLOAD --> Q1{"Люди в радиусе есть?"}

    Q1 -->|сбой| DERR["Колода: ошибка загрузки"]
    DERR --> DECK
    Q1 -->|нет| DEMPTY["Колода: пусто"]
    Q1 -->|да| PERSON["Профиль другого человека"]

    DEMPTY --> Q2{"Расширить радиус?"}
    Q2 -->|да| DRADIUS["Колода: расширение радиуса"]
    DRADIUS --> Q1
    Q2 -->|нет| FEED["Лента запросов"]

    PERSON --> Q3{"Профиль внушает доверие?"}
    Q3 -->|нет| DECK
    Q3 -->|да| LIKE["Колода: лайк отправлен"]
    LIKE --> Q4{"Взаимный лайк?"}
    Q4 -->|нет| DECK
    Q4 -->|да| MATCH["Совпадение"]
    MATCH --> DIALOG

    DECK --> FEED
    FEED --> FLOAD["Лента запросов: загрузка"]
    FLOAD --> Q5{"Есть подходящие запросы?"}

    Q5 -->|сбой| FERR["Лента запросов: ошибка загрузки"]
    FERR --> FEED
    Q5 -->|да| PLANCARD["Карточка запроса"]
    Q5 -->|нет| FEMPTY["Лента запросов: пусто"]

    FEMPTY --> Q6{"Создать свой запрос?"}
    Q6 -->|нет| STUCK1(["ТУПИК: ни людей, ни планов — ушла"])
    Q6 -->|да| CREATE["Создание запроса"]

    CREATE --> Q7{"Опубликовалось?"}
    Q7 -->|сбой| CERR["Создание запроса: ошибка публикации"]
    CERR --> CREATE
    Q7 -->|да| MYPLANS["Мои планы"]
    MYPLANS --> RESP["Отклики на мой запрос"]
    RESP --> Q8{"Кто-нибудь откликнулся?"}
    Q8 -->|нет| RESPEMPTY["Отклики: пусто"]
    RESPEMPTY --> Q9{"Запрос ещё живой?"}
    Q9 -->|да| RESP
    Q9 -->|нет, 14 дней прошли| EXPIRED["Карточка запроса: погас"]
    EXPIRED --> Q10{"Продлить?"}
    Q10 -->|да| MYPLANS
    Q10 -->|нет| STUCK2(["ТУПИК: никто не позвал, запрос погас"])
    Q8 -->|да| CONFIRM["Отклики: подтвердить или отклонить"]
    CONFIRM --> DIALOG

    PLANCARD --> Q11{"Верифицирована?"}
    Q11 -->|нет| GATE["Диалог: гейт верификации"]
    GATE --> VERIFY["Верификация"]
    VERIFY --> Q12{"Распознали?"}
    Q12 -->|да| VOK["Верификация: пройдена"]
    VOK --> PLANCARD
    Q12 -->|нет| VFAIL["Верификация: не распознали"]
    VFAIL --> Q13{"Третья попытка?"}
    Q13 -->|нет| VERIFY
    Q13 -->|да| VMANUAL["Верификация: на ручной проверке"]
    VMANUAL --> STUCK3(["ТУПИК: ждёт человека, писать пока нельзя"])
    Q11 -->|да| ASK["Карточка запроса: отклик отправлен"]

    ASK --> WAIT["Карточка запроса: жду ответа"]
    WAIT --> Q14{"Автор ответил?"}
    Q14 -->|отклонил| REJECTED["Карточка запроса: отклик отклонён"]
    REJECTED --> FEED
    Q14 -->|молчит| STUCK4(["ТУПИК: ответа нет, статус не меняется"])
    Q14 -->|подтвердил| DIALOG["Диалог"]

    DIALOG --> Q18{"Как выходим из разговора?"}
    Q18 -->|договорились сами| Q15
    Q18 -->|зовём вдвоём в чужой запрос| PAIR["Позвать вдвоём в запрос"]
    Q18 -->|никак| DEADCHAT["Диалог: разговор без даты"]
    DEADCHAT --> STUCK5(["ТУПИК: переписка без плана"])

    PAIR --> Q19{"Второй согласился?"}
    Q19 -->|нет| DIALOG
    Q19 -->|да| Q20{"Автор подтвердил пару?"}
    Q20 -->|нет| DIALOG
    Q20 -->|да| Q15

    Q15{"Дата назначена?"} -->|нет| STUCK5
    Q15 -->|да| REMIND["Напоминание Still going?"]
    REMIND --> Q16{"Встреча состоялась?"}
    Q16 -->|нет| STUCK6(["ТУПИК: не дошли, компания не собралась"])
    Q16 -->|да| WHOMADE["Кто дошёл Who made it?"]
    WHOMADE --> OFFER["Предложение повторить"]
    OFFER --> Q17{"Хочу повторить с ними же?"}
    Q17 -->|нет| WIN1(["Одна встреча состоялась — M1 не закрыт"])
    Q17 -->|да| REPEAT["Карточка запроса: серия с историей"]
    REPEAT --> WIN2(["Повтор с теми же людьми — M1 двигается"])
```

**Точки решения.**

1. **Люди в радиусе есть?** — на старте чаще «нет»: плотность главный риск (`D-02`).
2. **Расширить радиус?** — по `D-11` спрашиваем один раз, дальше помним.
3. **Профиль внушает доверие?** — решают четыре сигнала до первого сообщения (правило 12).
4. **Взаимный лайк?** — **вероятность, а не действие**, и это цена `D-37`: основной путь
   упирается в чужое решение. Второй путь через ленту от взаимности не зависит.
5. **Есть подходящие запросы?** / **Создать свой?** — развилка, где ломается или держится `D-22`.
6. **Верифицирована?** — гейт стоит перед контактом, не на входе (`D-03`).
7. **Распознали?** и **третья попытка?** — пересдача безлимитная, после двух неудач человек (`D-26`).
8. **Автор ответил?** — асимметрия отклика: место не занято, пока не подтвердил.
9. **Как выходим из разговора?** — три выхода: договориться самим, позвать друг друга
   вдвоём в чужой запрос (`D-36`), или никак. Третий — провал R1.
10. **Дата назначена?** — без даты нечего напоминать и нечего засчитывать (`D-23`).
11. **Хочу повторить?** — единственный переход, который двигает M1 по существу.
    Предложение приходит **отдельно от check-in** и доступно любому участнику (`D-34`):
    экран «Кто дошёл» стоит рядом на схеме, но повтор от него не зависит.

**Состояния в потоке.** Загрузка колоды и ленты · пусто в колоде, ленте и откликах ·
ошибка загрузки колоды и ленты · ошибка публикации запроса · расширение радиуса ·
гейт верификации · не распознали · на ручной проверке · жду ответа · отклик отклонён ·
запрос погас · **разговор без даты** · ждёт согласия второго.

**Шесть тупиков.** Ни людей, ни планов · никто не откликнулся и запрос погас ·
ждёт ручной проверки · автор молчит · переписка без плана · не дошли.
**Пятый и шестой — самые дорогие:** там человек уже вложился, а компании нет.

---

## R2 — решиться идти

> Когда мы уже списались и пора договариваться о встрече — а всё, что я о нём знаю,
> он рассказал о себе сам, — я хочу понимать, чем рискую, до того как соглашусь
> на место и время.

**Кто:** Даша, primary.

```mermaid
flowchart TD
    A(["В чате зашла речь о встрече"]) --> B["Карточка запроса"]
    B --> C{"Кто автор?"}
    C -->|профиль неполный| D["Профиль другого человека"]
    C -->|сигналы на карточке видны| E{"Verified есть?"}

    D --> E
    E -->|нет| F{"Идти без верификации автора?"}
    F -->|нет| G(["ТУПИК: не пошла, автор не проверен"])
    F -->|да| H{"Место публичное?"}
    E -->|да| H

    H -->|нет, приватный адрес| I["Карточка запроса: предупреждение о приватном месте"]
    I --> J{"Всё равно идти?"}
    J -->|нет| K(["ТУПИК: не пошла, место не устраивает"])
    J -->|да| L["Карточка запроса: отклик отправлен"]
    H -->|да| L

    L --> M["Карточка запроса: жду ответа"]
    M --> N{"Автор подтвердил?"}
    N -->|нет| O["Карточка запроса: отклик отклонён"]
    O --> P(["Вернулась в ленту"])
    N -->|да| Q["Диалог"]

    Q --> R{"Хочу ограничить, кто идёт?"}
    R -->|да, только свой пол| S["Создание запроса: кто может откликнуться"]
    R -->|нет| T["Ссылка близкому Share my plans"]
    S --> T
    T --> U{"Тревожно после разговора?"}
    U -->|да| V["Жалоба"]
    V --> W["Блокировка"]
    W --> X(["Контакт прекращён, человек в списке заблокированных"])
    U -->|нет| Y["Safety center"]
    Y --> Z(["Пошла на встречу — риск был понятен заранее"])
```

**Точки решения.**

1. **Кто автор?** — если четырёх сигналов на карточке мало, человек уходит в профиль целиком.
2. **Verified есть?** — единственный сигнал, который что-то доказывает; и то только живое фото (`D-26`).
3. **Место публичное?** — публичное по умолчанию, приватное показывает предупреждение (`D-07`).
4. **Хочу ограничить, кто идёт?** — только на свой пол (`D-25`); ветка работает, если она автор.
5. **Тревожно после разговора?** — Report и Block доступны с любого экрана (правило 4).

**Состояния.** Предупреждение о приватном месте · жду ответа · отклик отклонён.

**Два тупика, и оба — осознанный отказ, а не сбой:** не пошла, потому что автор
не верифицирован; не пошла, потому что место не устраивает. Продукт их не «чинит» —
он делает так, чтобы решение принималось **дома**, а не у двери.

---

## R4 — увидеться снова, не начиная заново

> Когда встреча прошла хорошо и я хочу повторить — а договариваться заново с каждым
> значит снова писать первой, — я хочу продолжить с теми же людьми без усилия первого раза.

**Кто:** Максим и Оля (secondary), Даша (primary) — источник Холла измерен на переехавших.

```mermaid
flowchart TD
    A(["Встреча прошла"]) --> B["Кто дошёл Who made it?"]
    B --> C{"Ответила в окно 14 дней?"}
    C -->|нет| D["Кто дошёл: окно закрыто"]
    D --> E(["ТУПИК: встреча не засчитана, счётчик не вырос"])
    C -->|да| F{"Кого-то отметили в ответ?"}
    F -->|нет| G(["ТУПИК: присутствие не подтверждено"])
    F -->|да| H["Мой профиль: счётчик встреч вырос"]

    H --> I{"Повторить с теми же?"}
    I -->|нет| J["Лента запросов"]
    J --> K(["Ищет новую компанию — R4 не закрыт"])
    I -->|да| L["Предложение повторить"]

    L --> M{"Кто-то из участников согласился?"}
    M -->|нет, предложение погасло| M2["Предложение повторить: погасло"]
    M2 --> R
    M -->|да| P["Карточка запроса: серия с историей"]
    P --> Q["Диалог: чат серии"]
    Q --> O{"Пришёл кто-то из прошлого состава?"}

    O -->|нет| R(["Состав новый — повтора не вышло"])
    O -->|да| S["Напоминание Still going?"]
    S --> T{"Дошли во второй раз?"}
    T -->|нет| U(["ТУПИК: серия распалась на второй встрече"])
    T -->|да| V(["Пара с двумя встречами — метрика повтора выросла"])
```

**Точки решения.**

1. **Ответила в окно?** — окно 14 дней, пропустить можно; при заполняемости ниже 40 %
   механизм репутации не наполняется (`D-18`).
2. **Кого-то отметили в ответ?** — присутствие засчитано, если отметил хотя бы один другой.
3. **Кто-то согласился?** — предложение повторить приходит **после встречи любому участнику**
   и не зависит от check-in (`D-34`). Прежняя развилка «запрос был помечен как повторяющийся?»
   снята: она спрашивала о повторе до того, как человек мог о нём судить, и роняла R4 тихо.
4. **Пришёл кто-то из прошлого состава?** — вторичный срез метрики повтора (§11).

**Состояния.** Окно check-in закрыто · предложение повторить погасло · серия с историей · чат серии.

**Три тупика.** Не ответила в окно · присутствие не подтвердил никто ·
серия распалась на второй встрече. Первые два — **дыры в измерении**: встреча была,
но продукт о ней не знает, и метрика повтора читается как нижняя граница.

---

## R3 — не приехать к пустому столу

> Когда я решаю, ехать ли на встречу, где записалось восемь незнакомых людей, —
> я хочу быть уверенной, что не окажусь там одна.

**Кто:** Оля и Артём — **secondary-flow** `D-35`. На `+1` пустого стола не бывает:
вопрос «сколько подтвердило» вырождается в «только я или никто».
**Но экран напоминания рисуется сначала под пару** — там молчание единственного участника
означает, что встречи не будет вовсе (`D-24`).

```mermaid
flowchart TD
    A(["За сутки до встречи"]) --> B["Напоминание Still going?"]
    B --> C{"Ответила?"}

    C -->|нет| D["Карточка запроса: счётчик going"]
    C -->|Can't make it| E["Карточка запроса: место освободилось"]
    E --> F(["Вышла заранее — честнее молчаливой неявки"])
    C -->|Yes| G["Карточка запроса: счётчик confirmed"]

    D --> H{"Молчание освобождает место?"}
    H -->|нет, по правилу| G

    G --> I{"Сколько подтвердило?"}
    I -->|только я| J["Карточка запроса: 1 confirmed"]
    J --> K{"Ехать одной?"}
    K -->|нет| L(["ТУПИК: не поехала, компания не собралась"])
    K -->|да| M["Кто дошёл Who made it?"]
    I -->|двое и больше| M

    M --> N{"Кто-то дошёл кроме меня?"}
    N -->|нет| O(["ТУПИК: приехала к пустому столу — job провален"])
    N -->|да| P(["Встреча состоялась, присутствие засчитано"])
```

**Точки решения.**

1. **Ответила?** — три исхода, и молчание не равно отказу (`D-16`).
2. **Молчание освобождает место?** — **нет**: неответ не проступок, `spots left` не растёт.
3. **Сколько подтвердило?** — на `+1` ответ всегда «только я или никто»,
   поэтому этот flow и не про Дашу `D-35`. Механизм при этом на паре не слабее,
   а сильнее: один неответ означает, что встречи нет.
4. **Кто-то дошёл кроме меня?** — единственная проверка, которая закрывает job фактом.

**Состояния.** Счётчик `going` · счётчик `confirmed` · место освободилось · `1 confirmed`.

**Два тупика.** Не поехала, потому что подтвердил один · приехала к пустому столу.
**Второй — это провал job целиком**, и продукт против него может только предупредить
заранее: показать `confirmed`, а не `going` (`D-16`).

---

## Что эти flows показали

| Находка | Куда идёт |
|---|---|
| **Шесть тупиков в M1**, два из них после того, как человек уже вложился | Проектировать выход из «автор молчит» и «переписка без плана» — §5.4 |
| **Ошибки загрузки и публикации** не были описаны как состояния | Добавлены в `sitemap.md` |
| **«Отклик отклонён» и «отклики: пусто»** не были состояниями | Добавлены в `sitemap.md` |
| ~~R4 ломается на развилке «запрос не был помечен повторяющимся»~~ | **Решено `D-34`:** повтор предлагается после встречи, начать может любой участник |
| **R3 на `+1` вырождается** — подтверждает, что flow secondary | Перепроверить владение R3 на интервью |
